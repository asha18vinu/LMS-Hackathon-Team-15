...
public class ManageProgramPage {
	WebDriver driver;
	 ...
	 public void clickCheckbox() {
		...
		public void deleteFunction(String name, WebElement  element1, WebElement element2,WebElement  element3 ) throws InterruptedException {
			element1.click();
			element2.click();
			element3.click();
...
//			Assert.assertEquals(message, "Programs Deleted");
			...
			Assert.assertEquals(exp, "Showing 0 to 0 of 0 entries", "Program deleted");
				LoggerLoad.info("Program deleted successfully");
		}
		


	public void deleteFunctionWithNoOption(String name, WebElement  element1, WebElement element2,WebElement  element3 ) throws InterruptedException {
		element1.click();
		element2.click();
		element3.click();
//		String message = successMessage.getText();
...
//	}
	...
	public void sortAscendingOrder(List<WebElement> list ) {
		try {
			...
			List<String> sortedList = originalList.stream().sorted(String.CASE_INSENSITIVE_ORDER).collect(Collectors.toList());
			
			System.out.println("*****Java Stream Sorting********" +sortedList);
			
			Assert.assertTrue(originalList.equals(sortedList));
			
			List<String> programDesc = list.stream().filter(s->s.getText().contains("Team22")).map(s->getPogramDescription(s)).collect(Collectors.toList());
			programDesc.forEach(a->System.out.println(a));
			
			LoggerLoad.info("Column Sorted in ascending order");
			
			//list.stream().filter(s-> s.getText().contains("Automation ")).map(s->getPogramDescription().collect(Collectors.toList());
	...
	public void sortDescendingOrder(List<WebElement> list) {
		try {
			...
			//Sort the original list --> sorted list
			List<String> sortedList = originalList.stream().sorted(String.CASE_INSENSITIVE_ORDER.reversed()).collect(Collectors.toList());
			System.out.println("*****Java Stream Sorting********" +sortedList);
...
//			programStatus.forEach(a->System.out.println(a));
			...
			//programNameList.stream().filter(s-> s.getText().contains("abcd")).map(s->getPogramDescription().collect(Collectors.toList()));
	...
	public void sortAscendingOrderInteger(List<WebElement> list ) {
		try {
			...
			//Sort the original list --> sorted list
			List<Integer> sortedList = originalList.stream().map(Integer::valueOf).sorted().collect(Collectors.toList());
			System.out.println("*****Java Stream Sorting********" +sortedList);
